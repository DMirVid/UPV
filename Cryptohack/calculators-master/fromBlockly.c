...
static void showBlocklyError(int rc)
{
  ...
  case BLOCKLY_NO_OUTPUT_EXPECTED:
    ...
    return;
  case BLOCKLY_CANNOT_PARSE_XML:
    SEND_DATA_TO_OUTPUT(lang ? "KError interno: no se puede analizar el XML generado por Blockly" :
      "KInternal Error: cannot analyze XML generated by Blockly");
    return;
  default:
    return;
  }
    ...
    else if (xmlcmp(ptrXML, "</statement") == 0)
    {
      while (ptrBlockStack > blockStack)
      {
        ...
        if (*ptrBlockStack == START_FOR)
        {
          ...
          if ((ptrBlockStack < &blockStack[2]) || (ptrStackControl < &stackIf[1]))
          {
            return BLOCKLY_CANNOT_PARSE_XML;
          }
          *ptrInstr = TOKEN_GET_VAR;
        ...
        if (*ptrBlockStack == START_REPEAT)
        {
          ...
          if ((ptrBlockStack < &blockStack[1]) || (ptrStackControl < &stackIf[1]))
          {
            return BLOCKLY_CANNOT_PARSE_XML;
          }
          *ptrInstr = TOKEN_GET_VAR;
